...
const SelectTradingPair = () => {
  ...
  const { tradingPairs, selectedTradingPair, selectPair, getPairs } = useExchange();
      ...
      <div className="inset-0 flex items-center">
        {
          ...
          >
            ...
            <div className="flex -space-x-5">
                <img className="w-10 h-10 mt-1 border-2 bg-white border-gray-100 rounded-full dark:border-gray-800 z-10" src={selectedTradingPair.token1.icon} alt=""/>
                <img className="w-10 mt-4 h-10 border-2 border-gray-100 bg-white rounded-full dark:border-gray-800" src={selectedTradingPair.token2.icon} alt=""/>
            </div>
                  ...
                  <div className="mt-2 max-h-[500px] overflow-y-scroll">
                    {
                      ...
                      tradingPairs.map((pair, index) => (
                        <div className="flex items-center space-x-2 border-b py-2 border-gray-50 hover:bg-gray-50 px-2 cursor-pointer" key={index} onClick={() => selectTradingPair(pair)}>
                          <div className="flex -space-x-5">
                              <img className="w-10 h-10 mt-1 border-2 bg-white border-gray-100 rounded-full dark:border-gray-800 z-10" src={pair.token1.icon} alt=""/>
                              <img className="w-10 mt-4 h-10 border-2 bg-white border-gray-100 rounded-full dark:border-gray-800" src={pair.token2.icon} alt=""/>
                          </div>
                            {/* <img src={require("../../data/cryptos/wbtc.png")} className="w-10 h-10 rounded-full"/> */}
                            <div className="flex flex-col items-start">
                              <span>{pair.name}</span>
