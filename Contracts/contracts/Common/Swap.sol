...
contract Swap {
    ...
    function swapExactInputMultihop(uint256 amountIn, address inputToken, address outputToken, address recipient) external returns (uint256 amountOut) {
        ...
        ISwapRouter.ExactInputParams memory params =
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(inputToken, poolFee, USDC, outputToken),
                recipient: recipient,
                deadline: block.timestamp,
                amountIn: amountIn,
                amountOutMinimum: 0
